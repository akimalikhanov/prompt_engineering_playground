...
def extract_input_text(messages: Any, max_length: int = 2000) -> str | None:
    ...
    try:
        if isinstance(messages, str):
            ...
            last_any_content: str | None = None
            for msg in messages:
                content = None
                role = "user"
...
def extract_system_prompt(messages: Any, max_length: int = 4000) -> str | None:
...
def extract_context_prompt(messages: Any, max_length: int = 4000) -> str | None:
    ...
    *,
    ...
    log_run(
        ...
        params=(
            ...
            if (params_obj := getattr(body, "params", None)) and hasattr(params_obj, "model_dump")
...
def log_run(
    ...
    accumulated_text: str | None = None,  # Alias for output_text (for streaming)
) -> int | None:
    """
    ...
    try:
        provider_key_val = provider_key or ""
        model_id_val = model_id or ""

        run = Run(
            trace_id=trace_id,
            request_id=request_id or trace_id,  # Use trace_id as fallback
            session_id=session_id,
            provider_key=provider_key_val,
...
def get_run_by_id(run_id: int) -> dict[str, Any] | None:
    ...
    session = SessionLocal()
    try:
        run: Any = session.query(Run).filter(Run.id == run_id).first()
