...
from utils.errors import BackendError, _get_status_code, prettify_openai_error
...
def route_call(
    ...
    params,
    stream=True,
    stream_mode: Literal["raw", "sse"] = "sse",
    max_retries=3,
    trace_id=None,
    session_id=None,
    ...
    if model_config["provider"] != provider_id:
        raise ValueError(
            f"Provider ID '{provider_id}' does not match model '{model_id}' provider '{model_config['provider']}'"
        )
    ...
    if provider_id not in providers:
        ...
        raise ValueError(
            f"Provider ID '{provider_id}' not supported. Available providers: {available_providers}"
        )
    ...
    try:
        ...
        raise BackendError(msg, status_code) from e
