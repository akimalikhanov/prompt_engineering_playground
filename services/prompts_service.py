...
from models.prompts import PromptExample
...
def get_latest_prompts(
    technique: str | None = None, q: str | None = None, is_active: bool | None = None
) -> list[PromptExample]:
    """
    ...
    try:
        ...
        result = session.execute(
            query, {"technique": technique, "is_active": is_active, "q_pattern": q_pattern}
        )
...
def get_prompt_by_id(prompt_id: str) -> PromptExample | None:
    """
    ...
    session = SessionLocal()
    try:
        prompt = session.query(PromptExample).filter(PromptExample.id == prompt_id).first()
...
def create_new_version(
    ...
    try:
        # Get the base prompt
        base_prompt = session.query(PromptExample).filter(PromptExample.id == prompt_id).first()
        ...
        max_version = (
            session.query(PromptExample)
            .filter(PromptExample.key == base_prompt.key)
            .order_by(desc(PromptExample.version))
            .first()
        )
        ...
        if auto_deactivate_previous:
            previous_active = (
                session.query(PromptExample)
                .filter(PromptExample.key == base_prompt.key, PromptExample.is_active.is_(True))
                .first()
            )

            if previous_active:
                previous_active.is_active = False  # type: ignore[assignment]
...
def get_latest_by_key(key: str) -> PromptExample | None:
    """
    ...
    try:
        ...
        result = session.execute(query, {"key": key}).first()
...
def get_prompt_by_id_and_version(
    prompt_id: str, version: int | None = None
) -> PromptExample | None:
    """
    ...
    try:
        if version is None:
            ...
            base = session.query(PromptExample).filter(PromptExample.id == prompt_id).first()
            ...
            prompt = (
                session.query(PromptExample)
                .filter(PromptExample.key == base.key, PromptExample.version == version)
                .first()
            )
...
def update_prompt_status(prompt_id: str, is_active: bool | None = None) -> PromptExample | None:
    """
    ...
    try:
        ...
        if is_active is not None:
            prompt.is_active = is_active  # type: ignore[assignment]
