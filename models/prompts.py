...
class PromptExample(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            ...
            name="prompt_examples_technique_check",
        ),
        CheckConstraint(
            "response_format IN ('json_object', 'json_schema')",
            name="prompt_examples_response_format_check",
        ),
        {"schema": "app"},
    )
    ...
    json_schema_template = Column(
        JSONB
    )  # JSON schema definition (only used when response_format='json_schema')
