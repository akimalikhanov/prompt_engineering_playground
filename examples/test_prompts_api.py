...
class PromptsAPIClient:
    ...
    def list_prompts(self, technique_key: Optional[str] = None, 
                    q: Optional[str] = None, 
    ...
    def create_prompt(self, technique_key: str, title: str, messages: list,
    ...
    def create_version(self, example_id: str, 
        ...
        response = requests.post(
            f"{self.base_url}/prompts/{example_id}/versions", 
            json=data
        )
        response.raise_for_status()
        return response.json()
    ...
    def get_latest(self, technique_key: str, title: str):
        ...
        response = requests.get(
            f"{self.base_url}/prompts/{technique_key}/{title}/latest"
        )
        response.raise_for_status()
        return response.json()
    
    def render_prompt(self, example_id: str, variables: dict, 
        ...
        response = requests.post(
            f"{self.base_url}/prompts/{example_id}/render",
            json=data,
            params=params
        )
        response.raise_for_status()
    ...
    def update_prompt(self, example_id: str, 
        ...
        response = requests.patch(
            f"{self.base_url}/prompts/{example_id}",
            json=data
        )
        response.raise_for_status()
        return response.json()
...
def main():
    ...
    try:
        rendered = client.render_prompt(
            prompt_id,
            variables={"concept": "list comprehension"}
        )
        print(f"   Missing vars: {rendered['missing_vars']}")
    ...
    if cot_prompts['count'] > 0:
        ...
        if variables_needed:
            ...
            rendered = client.render_prompt(
                cot_prompt['example_id'],
                variables={var_name: "42 * 13 + 7"}
            )
